...
struct nodecnst_data
{
	...
	// int constraint_type = 0; // 0 - Fixed support, 1 - Pinned support
...
public:
	...
	int ndcnst_type = 0; //0 - Fixed, 1 - Pinned
	int model_type = 0; // 0,1 - Line, 2,3 - Circle

	nodecnst_list_store();
	~nodecnst_list_store();
	void init(geom_parameters* geom_param_ptr);
	void set_zero_condition(int ndcnst_type, const int& model_type);
	void add_nodeconstraint(int& node_id, glm::vec3& ndcnst_loc, glm::vec3& ndcnst_normals);
